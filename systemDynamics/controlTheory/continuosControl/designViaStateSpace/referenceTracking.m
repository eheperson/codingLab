...
% s-plane
%
Aclosedloop = (A-B*K); %Calculate new A matrix(closed loop response)
%
...
pNew = [-60, -65, -70];
% pNew = [-10, -20, -22];
% eger sistemin tRising maxOvershoot gibi parametrelerini degistirmek
% istersek pNwe matrisini degistirmemiz gerekiyor
% Simdilik rastgele bir sekilde 
% Degerlek cok yuksek secilir ise kontrolcu daha hizli cevap verir
% sistem daha iyi kontrol edilir
%
Knew = place(Anew, Bnew, pNew);
%
AnewClosedloop = (Anew - Bnew*Knew); %Calculate new A matrix(closed loop response)
%
eig(AnewClosedloop) % check if new poles are same with 'p'
%
