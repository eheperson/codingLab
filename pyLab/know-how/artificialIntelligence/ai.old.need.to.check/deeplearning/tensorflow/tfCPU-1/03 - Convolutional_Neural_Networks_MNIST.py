...
#mnist dimensions : 28x8x1
x = tf.placeholder(tf.float32, [None, 28, 28, 1]) #28x28x21
...
# Convolutionallayers
weight1 = tf.Variable(tf.truncated_normal([5, 5, 1, filter1], stddev=0.1)) #filter1 dimensions : 5x5x1 blackwhite image
...
# Layer to flate image (flatten layer)
weight3 = tf.Variable(tf.truncated_normal([7*7*filter2, 256], stddev=0.1))
bias3 = tf.Variable(tf.constant(0.1, shape=[256]))

# Fully Connected(Dense) Layer
weight4 = tf.Variable(tf.truncated_normal([256, 10], stddev=0.1))
bias4 = tf.Variable(tf.constant(0.1, shape=[10]))

# input = [28, 28, 1]                     strides=[batch, x, y, depth]
y1 = tf.nn.relu(tf.nn.conv2d(x, weight1, strides=[1, 1, 1, 1], padding='SAME') + bias1) # output = [28, 28, 16]
y1 = tf.nn.max_pool(y1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME') # output = [14, 14, 16]

y2 = tf.nn.relu(tf.nn.conv2d(y1, weight2, strides=[1, 1, 1, 1], padding='SAME') + bias2) # output = [14, 14, 32]
