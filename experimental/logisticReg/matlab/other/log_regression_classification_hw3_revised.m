close all
clear all
%%% CLASSIFICATION Problem
%% logistic regression application
%
%
%% Create Dataset
number_of_data = 100;

mu1 = [2,3];
sigma1 = [1,1.5;1.5,3];
rng default  % For reproducibility
rnd_number_1 = mvnrnd(mu1,sigma1,number_of_data);

mu2 = [5,4];
sigma2 = [1,1.5;1.5,3];
rng default  % For reproducibility
rnd_number_2 = mvnrnd(mu2,sigma2,number_of_data);

figure
plot(rnd_number_1(:,1),rnd_number_1(:,2),'+')
hold on
plot(rnd_number_2(:,1),rnd_number_2(:,2),'o')
%
%
%% Create DataSet Matrix

X0 = ones(2*length(rnd_number_1(:,1)),1);
X1 = [rnd_number_1(:,1);rnd_number_2(:,1)];
X2 = [rnd_number_1(:,2);rnd_number_2(:,2)];

X_general_unnormalized = [X0 X1 X2];
X_general = [X0 mat2gray(X1) mat2gray(X2)];

Y_general = [zeros(length(rnd_number_1(:,1)),1);ones(length(rnd_number_1(:,1)),1)];

training_number = length(X0);

%% Model
% firstly, model function H(x) should be defined to train the network.
% H(x) = 1/(1 + exp(teta_0*x0 + teta_1*x1 + teta_2*x2))

theta = randi([-20 100],3,1);

%% Error Function 

error_func = -1./(training_number).*(Y_general.*log(1./(1 + exp(-(theta(1,1).*X_general(:,1) + theta(2,1).*X_general(:,2) + theta(3,1).*X_general(:,3)))))...
                                  + (1 - Y_general).*log(1 - 1./(1 + exp(-(theta(1,1).*X_general(:,1) + theta(2,1).*X_general(:,2) + theta(3,1).*X_general(:,3))))));        

                              
learning_rate = 1;
epsilon = 1e-3*ones(length(X0),1);

iteration = 0;
iterMax = 20000;

%% Animate Learning Results
figure
ax1 = subplot(3,1,1); %mean error
h1 = animatedline;

ax2 = subplot(3,1,2); %delta error
h2 = animatedline;

ax3 = subplot(3,1,3); %learning rate
h3 = animatedline;

%% Learning Algorithm
while (sum(abs(error_func(:,1)) > epsilon ) ~= 0) && iterMax > iteration
    
    iteration = iteration + 1;
    
    internal_error_func = error_func; % That line is foranimation
    
    for i = 1:1:training_number
       h(i) = 1./(1 + exp(-(theta(1,1).*X_general(i,1) + theta(2,1).*X_general(i,2) + theta(3,1).*X_general(i,3))));
       
       temp1 = theta(1,1) - learning_rate*( h(i) - Y_general(i,1))*X_general(i,1);
       
       temp2 = theta(2,1) - learning_rate*( h(i) - Y_general(i,1))*X_general(i,2);
       
       temp3 = theta(3,1) - learning_rate*( h(i) - Y_general(i,1))*X_general(i,3);
        
       theta(1,1) = temp1;
       theta(2,1) = temp2;
       theta(3,1) = temp3;
       
       error_func(i, 1) = -1/(training_number)*(Y_general(i,1)*log(1/(1 + exp(-(theta(1,1)*X_general(i,1) + theta(2,1)*X_general(i,2) + theta(3,1)*X_general(i,3)))))...
                                  + (1 - Y_general(i,1))*log(1 - 1/(1 + exp(-(theta(1,1)*X_general(i,1) + theta(2,1)*X_general(i,2) + theta(3,1)*X_general(i,3)))))); 
   
    
    end
    
    mean(error_func) %% Print out error mean to console
    
    y_axis_lim = sum(error_func);
    
    xlim(ax1, [iteration - 50, iteration + 50]);
    ylim(ax1, [y_axis_lim/5 y_axis_lim*5]);
    addpoints(h1, iteration, sum(error_func));
    drawnow limitrate
    
    if sum(isinf(internal_error_func)) == 0
        
        y_axis_lim = sum(internal_error_func) - sum(error_func);
    
        xlim(ax2, [iteration - 50, iteration + 50]);
        ylim(ax2, [y_axis_lim/5 y_axis_lim*5]);
        addpoints(h2, iteration, sum(internal_error_func) - sum(error_func));
        drawnow limitrate 
    end
    
    addpoints(h3, iteration, learning_rate);
    drawnow limitrate
end
%% then, cost function should be prepared in order to be appropriate form
%% J(teta) = 1/m*summation((h(x) - y)^2)
%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Your code should be in here.






%% Your code should be in here.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%




h_teta_x = 1./(1 + exp(-(theta(1,1).*X_general(:,1) + theta(2,1).*X_general(:,2) + theta(3,1).*X_general(:,3))));

figure
plot([1:1:training_number],Y_general);
figure
plot([1:1:training_number],h_teta_x);


% for plotting boundary section in graph
% figure
% plot(rnd_number_1(:,1),rnd_number_1(:,2),'+')
% hold on
% plot(rnd_number_2(:,1),rnd_number_2(:,2),'o')
% 
% x1_boundary = abs(teta(1,1)/(teta(2,1)))*max(X_general_unnormalized(:,2));
% x2_boundary = abs(teta(1,1)/(teta(3,1)))*max(X_general_unnormalized(:,3));
% plot([0 x1_boundary*3],[x2_boundary*3 0]);

plotDecisionBoundary(theta, X_general, Y_general)


coeff = 0;



% end